namespace FormulaOne
{
    internal class Program
    {
        ...
        {
           Chempionship championship = new Chempionship();
            ...
            // getDrivers
            List<Driver> driverCollection = championship.getDrivers();
            bool test1 = driverCollection.Contains(driver_Ali);
            if (!test1)
            {
                Console.Error.WriteLine("test1: Haydovchi 'Ali'   topilmadi ERROR");
          
            }

            bool test2 = driverCollection.Contains(driver_Bob);
            if (!test2)
            {
                Console.Error.WriteLine("test2: Haydovchi 'Bob' topilmadi ERROR");
            ...
            if (test3 == null)
            {
                Console.Error.WriteLine("test3: Voy  'Afandi' topilmadiyu ERROR");
            }

            Driver test4 = championship.getDriver("Qorbobo");
            if (test4 != null)
            {
                Console.Error.WriteLine("test4: Voy  'Qorbobo' qattan keldi ERROR");
            }
